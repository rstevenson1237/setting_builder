# Lore.md

## Purpose
A registry of written works discoverable within the setting - books, scrolls, tapestries, notes, correspondence, maps, and the like - that reveal purpose behind a region, connect its History and Truths, and offer adventure hooks. Generated in two phases: stubbed empty at step 2h, a stub row added per entry during location generation (step 4c), and the full entry written afterward (step 4d).

## Context
Consult when drafting:
- Step 2h (stubbing the file): no context needed - the file starts with just its title line, no entries.
- Step 4c (recording a stub): `GENRE.md` and the location Feature that calls for a piece of Lore - see `templates/Location.md`.
- Step 4d (writing the full entry): `GENRE.md`, `setting/Setting.md`, `setting/History.md`, `setting/Truths.md`, `patterns/Lore.md`, and the location file the stub points to.

## Instructions
- **2h**: create the file with only its title line.
- **4c**: append a stub row - Title and Location only - when a location's Feature calls for a piece of Lore, and cite it in that Feature.
- **4d**: replace each stub row with its full entry per `patterns/Lore.md` - what the work is, what it reveals, and how it connects to History or Truths.

## Template
```
Lore of [Setting Name]

[Title] (*form - book, scroll, tapestry, note, letter, map, etc.*) - found at [Location Code] [Location Name]
[Full content, written in step 4d: what it reveals, how it connects to History/Truths, what hook it offers]
```
