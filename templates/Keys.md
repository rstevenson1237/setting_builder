...
## Purpose
A registry of physical objects that trigger something elsewhere - an actual key, a rod, a fitted gemstone, and the like. Generated in two phases: stubbed empty at step 2i, a stub row added per entry during location generation (step 4c), and the full entry written afterward (step 4d).

## Context
Consult when drafting:
- Step 2i (stubbing the file): no context needed - the file starts with just its title line, no entries.
- Step 4c (recording a stub): `GENRE.md` and the location Feature that calls for a Key - see `templates/Location.md`.
...
Unlocks: [what it unlocks or triggers, written in step 4d: the specific Feature and Location Code]
```
