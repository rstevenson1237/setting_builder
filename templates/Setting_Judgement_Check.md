...
- **One fact points past the edge of the map** - per `GENRE.md`, does something in this region name a place, a person, or a road this setting does not contain, and does it hang on something a party can ask after, follow, or pick up? Two failures, and the second is the common one: no such fact anywhere in the region, so the map reads as the whole world; or three of them, so a lead stops reading as a lead. A fact something elsewhere in the setting answers is not one of these - that is a Key or a piece of Lore with both ends written, and **Recurring elements actually recur** is the item that judges it.
...
- One fact points past the edge of the map: [Confirmed / Needs Attention - note]
