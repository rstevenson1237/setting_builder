...
- **the class file for this location's rating and class** - `Settlement.md` for SAFE; `Landmark.md`, `Hidden.md`, or `Secret.md` for WILD by classification; `High.md`, `Medium.md`, or `Low.md` for DANGEROUS by weight. A class is defined by what it guarantees, never by a ceiling on what may appear. Its own Spec states, line by line, which other pattern file(s) that line draws from - read only the files a line actually cites, and only from this location's own rating folder; a file borrowed from another folder is guidance written for a different kind of place. Every class file's Dressing line is unconditional, not just read when a spec line calls for it. Dressing may originate content of its own: **Features owns anything that changes what players can do; Dressing owns everything else.**
