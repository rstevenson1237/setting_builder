...
- **The three tiers are supplied, and each drawing class states its own triple** - per `templates/Location.md` a location displays information at three tiers, and the pattern library is what fills them. For each class file, does its Spec draw something **obvious** (Dressing, a visible challenge, a reason to stop), something at the **trigger** tier that rewards acting on what is obvious, and - at whatever rate it sets - a **secret**: a concealed detail whose Clue, Trigger and Payload that file states itself? A class drawing a concealed detail without stating its own triple has lost the customization the distribution was for, and a class whose Spec reaches only one tier will produce flat entries however good the lists it draws from are. Check the element files against the same three: a file supplying only read-aloud sits at one tier and is the **Interactive** finding below.
