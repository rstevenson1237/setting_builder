...
- **Correct patterns, correct order** - does the template's Context section list every pattern file actually relevant to that artifact, in the order they'd be consulted at generation time (e.g. `GENRE.md` first; unconditional patterns like `Dressing.md`/`Secrets.md` before the rating/weight-specific pattern; element files like `Treasure.md`/`Creature.md`/`Trap.md`/`Mystery.md` only after a chosen spec line calls for them)? Does it name only files that exist **in the rating folder it is describing** - the three folders do not hold the same set, and `patterns/safe/` in particular is thinner than the others?
