...
- **Correct patterns, correct order** - does the template's Context section list every pattern file actually relevant to that artifact, in the order they'd be consulted at generation time (e.g. `GENRE.md` first; unconditional patterns like `Dressing.md`/`Secrets.md` before the rating/weight-specific pattern; element files like `Treasure.md`/`Creature.md`/`Hazard.md`/`Mystery.md` only after a chosen spec line calls for them)? Does it name only files that exist **in the rating folder it is describing** - the three folders do not hold the same set, and `patterns/safe/` in particular is thinner than the others?
...
- **Format edge cases preserved** - do the format-sensitive templates (`templates/Location.md` above all) still state the specific rules that have previously needed calling out: header line format, Player Summary bolding, Referee Notes in italics, Feature labels without a leading article, an Exit needing a trigger belongs in a Feature not under Exits, treasure is cited as a single d20 pull and never named directly, units split feet (indoor)/yards (outdoor) with vertical drops always in feet, Features and Exits state a position (wall/corner/center, or a direction outdoors) and Features state their own dimension when spatially significant, WILD locations carry a landmark/hidden/secret classification (DANGEROUS: low/medium/high; SAFE: neither) and generate landmark tier before hidden or secret?
