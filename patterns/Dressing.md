...
- A former purpose is a source of detail on its own (tool marks, worn grooves, leftover fixtures) even once the room's function has changed or decayed.

## Sounds and Smells
- Every room carries at least one sensory detail beyond the visual - a smell, a sound, a temperature, a texture underfoot.
- Favor a detail that reinforces the region's theme or the room's purpose over a generic one.
...
- Every exit also states a position, for the same reason a Feature does - which wall or corner it opens from, or which direction it leads outdoors. This is baseline, not a detail that scales with weight: without it, an exit can't be placed at the table.
- When two exits in the same room share a type (two low stone passages, two plain doors), their stated positions are what keeps them distinguishable - never leave two exits reading as identical with nothing but their destination to tell them apart.
...
- **High weight** - dressing itself becomes exploration material. Multiple out-of-spec details, and where one gives the players something to actually interact with (not just read past), pull it out of Referee Notes into its own Feature line.
